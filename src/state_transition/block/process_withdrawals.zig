...
pub fn getExpectedWithdrawals(
    ...
    if (comptime fork.gte(.gloas)) {
        var result = try getBuilderWithdrawals(
            allocator,
            state,
            &withdrawal_index,
        ...
        defer result.withdrawals.deinit(allocator);
    ...
    if (comptime fork.gte(.electra)) {
        var result = try getPendingPartialWithdrawals(
            fork,
            allocator,
            epoch,
            state,
    ...
    if (comptime fork.gte(.gloas)) {
        var result = try getBuildersSweepWithdrawals(
            allocator,
            state,
            epoch,
            &withdrawal_index,
    ...
    {
        var result = try getValidatorsSweepWithdrawals(
            fork,
            allocator,
            epoch,
            state,
        ...
        withdrawals_result.sampled_validators = result.processed_count;
    }
...
fn getPendingPartialWithdrawals(
    ...
    var pending_partial_withdrawals_result: std.ArrayList(Withdrawal) = .empty;
    errdefer pending_partial_withdrawals_result.deinit(allocator);
    ...
    for (0..pending_partial_withdrawals_len) |_| {
            ...
            balance > preset.MIN_ACTIVATION_BALANCE)
        {
            ...
            try pending_partial_withdrawals_result.append(allocator, .{
...
fn getValidatorsSweepWithdrawals(
    ...
    var sweep_withdrawals: std.ArrayList(Withdrawal) = .empty;
    errdefer sweep_withdrawals.deinit(allocator);
    ...
    for (0..validators_limit) |n| {
        ...
        if (isFullyWithdrawableValidator(fork, withdrawal_credentials, balance, withdrawable_epoch, epoch)) {
            ...
            try sweep_withdrawals.append(allocator, .{
                .index = withdrawal_index.*,
...
fn getBuilderWithdrawals(
    ...
    var builder_withdrawals: std.ArrayList(Withdrawal) = .empty;
    errdefer builder_withdrawals.deinit(allocator);

    var builder_pending_withdrawals = try state.inner.get("builder_pending_withdrawals");
    try builder_pending_withdrawals.commit();
    ...
    for (0..builder_pending_withdrawals_len) |_| {
        ...
        try builder_withdrawals.append(allocator, .{
            .index = withdrawal_index.*,
...
fn getBuildersSweepWithdrawals(
    ...
    var builders_sweep_withdrawals: std.ArrayList(Withdrawal) = .empty;
    errdefer builders_sweep_withdrawals.deinit(allocator);
    ...
    for (0..builders_limit) |n| {
        ...
        if (builder.withdrawable_epoch <= epoch and balance > 0) {
            ...
            try builders_sweep_withdrawals.append(allocator, .{
                .index = withdrawal_index.*,
                .validator_index = convertBuilderIndexToValidatorIndex(builder_index),
